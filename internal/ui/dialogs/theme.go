...
// Update handles the theme dialog state updates based on messages.
func (t *themeDialog) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, themeKeys.Enter):
			...
			// 	}
			// 	if err := theme.Set(selectedTheme); err != nil {
...
// View renders the theme dialog component
...
// BindingKeys returns the key bindings for the theme dialog
func (t *themeDialog) BindingKeys() []key.Binding {
	return layout.KeyMapToSlice(themeKeys)
}

// NewTheme creates a new instance of the theme dialog component
func NewTheme() ThemeDialog {
	return &themeDialog{
		themes:       []string{},
		selectedIdx:  0,
		currentTheme: "",
